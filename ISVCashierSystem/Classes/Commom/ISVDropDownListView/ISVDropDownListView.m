...
@interface ISVDropDownListView ()
@property (nonatomic, strong) UIView *mTableBaseView;
@property (nonatomic, strong) UITableView *mTableView;
@end

@implementation ISVDropDownListView
{
    UILabel *_nameLabel;
}
...
- (instancetype)initWithFrame:(CGRect)frame dataSource:(id)datasource delegate:(id)delegate
{
    ...
    if (self) {
        ...
        for (int i = 0; i <sectionNum; i++) {
            ISVHorizontalButton *sectionBtn = [[ISVHorizontalButton alloc] initWithFrame:CGRectMake(sectionWidth*i, 1, sectionWidth, frame.size.height-2)];
...
{
    static NSString *ISVDropDownListViewCell = @"ISVDropDownListViewCell";
    UITableViewCell *cell = [tableView cellForRowAtIndexPath:indexPath];
    if (!cell) {
        cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:ISVDropDownListViewCell];
