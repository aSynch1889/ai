...
*  @param rule   规则类型
*/
+ (void)showScanControllWithViewController:(UIViewController *)viewController forRule:(ISVQRCodeToolRule)toRule
{
    // 暂时不对指定的路由处理
...
{
    [ISVSystemTool captureDeviceAuthStatusWithSuccessCallback:^{
        
        ISVScanViewController *scanViewController = [[ISVScanViewController alloc] init];
        [scanViewController setCompleteBlock:^(NSString *str) {
            NSLog(@"ISVQRCodeTool扫描到：%@", str);
            
            if (!str || [str isKindOfClass:[NSNull class]]) {
                [ISVHUD showErrorWithStatus:@"不能识别的二维码"];
                return ;
            }
            //str  做截取
            NSRange range;
            ...
            if (range.location != NSNotFound) { // 加密过的
                ...
                [ISVNetworking userGetQRCodeInfoWithEncryptKey:[str substringFromIndex:range.length] success:^(id respondData) {
                    
                    if (HasValuse) {
                        NSDictionary *dict = Response_Valuse;
                        ISVQRCodeToolRule Key = [[dict objectForKey:@"Key"] integerValue];
                    ...
                    [ISVHUD showErrorWithStatus:error.errMsg];
                }];
                
            }else{ // 未加密过的
                ! completeBlock ? : completeBlock(ISVQRCodeToolRuleOther, str);
            }
        ...
        [ISVHUD showErrorWithStatus:@"扫描出错"];
    }];
}
...
//    [ISVNetworking userGetQRCodeInfoWithEncryptKey:[enRSA encryptByRsa:dataStr] success:^(id respondData) {
