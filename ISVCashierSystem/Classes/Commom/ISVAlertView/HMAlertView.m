...
// 定义AlertView任务处理类
@interface HMAlertViewStack : NSObject

@property (nonatomic) NSMutableArray *alertViews;

+ (HMAlertViewStack *)sharedInstance;

- (void)push:(HMAlertView *)alertView;
- (void)pop:(HMAlertView *)alertView;
...
- (void)show
{
    [[HMAlertViewStack sharedInstance] push:self];
}
...
- (void)dismiss:(id)sender animated:(BOOL)animated
{
    ...
    [UIView animateWithDuration:(animated ? 0.2 : 0) animations:^{
        ...
        if ([[[HMAlertViewStack sharedInstance] alertViews] count] == 1) {
            if (animated) {
        ...
        [[HMAlertViewStack sharedInstance] pop:self];
    }];
}
...
+ (instancetype)showAlertWithTitle:(NSString *)title
                       ...
                       cancelTitle:(NSString *)cancelTitle
                        completion:(HMAlertViewCompletionBlock)completion
{
    HMAlertView *alertView = [[self alloc] initWithTitle:title
                                                 message:message
                                             cancelTitle:cancelTitle
                                              otherTitle:nil
...
+ (instancetype)showAlertWithTitles:(NSArray *)otherTitles
                       contentView:(UIView *)view
                        completion:(HMAlertViewCompletionBlock)completion
{
    HMAlertView *alertView = [[self alloc]initWithTitles:otherTitles buttonsShouldStack:NO contentView:view completion:completion];
    [alertView show];
    return alertView;
}
...
@implementation HMAlertViewStack

+ (instancetype)sharedInstance
{
    static HMAlertViewStack *_sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _sharedInstance = [[HMAlertViewStack alloc] init];
...
- (void)push:(HMAlertView *)alertView
{
    for (HMAlertView *av in self.alertViews) {
        if (av != alertView) {
            [av hide];
        }
        else {
...
- (void)pop:(HMAlertView *)alertView
{
    ...
    HMAlertView *last = [self.alertViews lastObject];
