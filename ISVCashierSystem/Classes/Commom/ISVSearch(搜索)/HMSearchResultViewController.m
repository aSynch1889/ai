//
//  HMSearchResultViewController.m
//  HealthMall
//
...
@implementation HMSearchResultViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    HMSearchCell *cell = [tableView dequeueReusableCellWithIdentifier:ksearchResultCellID];
    if (cell == nil) {
        cell = [[[NSBundle mainBundle] loadNibNamed:NSStringFromClass([HMSearchCell class]) owner:nil options:nil] lastObject];
    }
    
    id<HMSearchPotocol> obj = self.searchResults[indexPath.row];
    cell.model = obj;
...
- (void)setSearchResults:(NSMutableArray<id<HMSearchPotocol>> *)searchResults
{
...
- (void)searchResultsUpdateWithSources:(NSArray<id<HMSearchPotocol>> *)sources searchText:(NSString *)searchText
{
    ...
    if (searchText.length == 0) {  // 没有输入任何关键词
        ...
        if (results.count == 0) {
            ...
            [HMHUD showPageWithPageType:HMHUDPageTypeNoData InView:self.tableView];
            return;
        }
