...
@interface ISVScoreView ()
@property (nonatomic, weak) ISVCoverButton *coverBtn;
/// 五角星宽度(高度)
...
@implementation ISVScoreView

+ (instancetype)scoreViewWithPoint:(CGPoint)point score:(CGFloat)score
{
    ISVScoreView *panelView = [[self alloc] initWithFrame:CGRectMake(point.x, point.y, kWidth, kHeight)];
...
- (void)addTarget:(id)target actionForTouchUpInside:(SEL)action
{
    ...
    {
        // 添加按钮
        ISVCoverButton *btn = [ISVCoverButton coverButtonWithFrame:CGRectMake(0, 0, kWidth, kHeight)];
