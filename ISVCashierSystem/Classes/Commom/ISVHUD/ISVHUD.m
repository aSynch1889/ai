...
@interface ISVHUD ()
@property (nonatomic, weak) ISVJumpProgressHUD *jumpHud;
@end

@implementation ISVHUD
...
+ (void)showPageWithPageType:(ISVHUDPageType)pageType InView:(UIView *)supperView;
{
    ...
    if (pageType == ISVHUDPageTypeNoConnect) {// 网络不佳
    ...
    }else if (pageType == ISVHUDPageTypeNoData){// 没数据
    ...
    }else if (pageType == ISVHUDPageTypeLoading){// 加载中
        UIView *covrer = self.coverView;
        ISVJumpProgressHUD *hud = [[ISVJumpProgressHUD alloc] initWithFrame:
                                  CGRectMake((covrer.width-112)*0.5, covrer.height*0.5-140, 112, 140)];
        hud.tag = ISVHUDPageTypeLoading;
...
+ (void)dismissPage
{
    ...
    ISVJumpProgressHUD *hud = [self.coverView.subviews lastObject];
    if ([hud isKindOfClass:[ISVJumpProgressHUD class]])
    {
        [hud stopWithBlock:^{
...
{
    [[NSNotificationCenter defaultCenter] postNotificationName:ISVHUDReLoadDataNotification object:nil];
}
