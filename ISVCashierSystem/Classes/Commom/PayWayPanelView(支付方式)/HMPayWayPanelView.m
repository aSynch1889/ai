...
@interface HMPayWayPanelView ()<UITableViewDataSource, UITableViewDelegate>
{
    BOOL isLoadMore;    // 是否加载更多
    NSUInteger _payWayPanelViewMaxCount;   // 最大行数
    HMPayWayModel *_lastModel;
}
@property (nonatomic, weak) UILabel *titleLabel;
@property (nonatomic, weak) UITableView *tableView;
@property (nonatomic, strong) NSArray<HMPayWayModel *> *dataList;
@end

@implementation HMPayWayPanelView
...
{
    static NSString *HMPayWayPanelCellID = @"HMPayWayPanelCellID";
    HMPayWayPanelCell *cell = [tableView dequeueReusableCellWithIdentifier:HMPayWayPanelCellID];
    if (cell == nil) {
        cell = [[HMPayWayPanelCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:HMPayWayPanelCellID];
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
    }
    
    if (indexPath.row == _payWayPanelViewMaxCount  && !isLoadMore) {
        
        HMHorizontalButton *btn = [HMHorizontalButton buttonwithImageName:@"icon_more_down" title:@"更多"];
        ...
        HMPayWayModel *model = self.dataList[indexPath.row];
        cell.model = model;
    }
...
- (void)requertPaymentList
{
    ...
    [HMNetworking getPaymentListWithSuccess:^(id respondData) {
        
        NSArray *arr = Response_Valuse;
        if (HasValuse && [arr isKindOfClass:[NSArray class]]) {
            weakSelf.dataList = [HMPayWayModel objectArrayWithKeyValuesArray:arr];
            ...
            for (NSUInteger i = 0; i < weakSelf.dataList.count; i++ ) {
                HMPayWayModel *m = weakSelf.dataList[i];
                if (m.ID == lastID) {
                    row = i;
                }
            }
