...
#import "ISVNetworking+UserCenter.h"
#import "NSString+ISVHash.h"

@implementation ISVNetworking (UserCenter)
...
+ (void)userLoginWithUserName:(NSString *)User_Name password:(NSString *)pwd success:(RespondBlock)success failure:(ErrorBlock)failure
{
    ...
    if ([User_Name isEqualToString:ISV_NULL])
    {
        isVisitor = @(YES);
    }
    else
    ...
    [self postWithURL:url params:param success:^(id respondData) {
        ...
        if (NotNilAndNull(token) && token.length) {
            [[[ISVNetworking alloc] init] performSelectorOnMainThread:@selector(setToken:) withObject:token waitUntilDone:YES];
        }
...
{
    [self userLoginWithUserName:ISV_NULL password:@"42a29777faaf3b0d7c8a" success:^(id respondData) {
        success(respondData);
    } failure:^(ISVErrorModel *error) {
        failure(error);
    }];
...
/*
 ...
 {
 "model": {
 "ISV_U_Telephone": "13900000010",
 "ISV_U_Pwd": "123456",
 "ISV_U_NickName": "湖南光头强",
 "ISV_UI_Sex": 1
 },
 "whichFunc": "ADDUSER"
 }
 ...
 参数	是否必须	描述
 model	是	传入实体名称
 model.ISV_U_Telephone	是	电话号码
 model.ISV_U_Pwd	是(手机端先进行
 MD5加密)	密码
 model.ISV_U_NickName	是	昵称
 model.ISV_UI_Sex	是	性别
 ...
 */
#pragma mark - 用户注册
+ (void)userRegisterWithTelephone:(NSString *)ISV_U_Telephone password:(NSString *)pwd nickname:(NSString *)ISV_U_NickName sex:(NSString *)ISV_UI_Sex type:(NSString *)type key:(NSString *)key ID:(NSString *)ID success:(RespondBlock)success failure:(ErrorBlock)failure
{
    NSString *whichFunc = @"ADDUSER";
    NSString *ISV_U_Pwd = [pwd md5];
    NSDictionary *model = NSDictionaryOfVariableBindings(ISV_U_Telephone, ISV_U_Pwd, ISV_U_NickName,ISV_UI_Sex);
...
/*
 ...
 "ISV_UI_City": "广州",
 "ISV_UI_Job": "IT程序猿",
 "ISV_UI_CaseHistory": "无",
 "ISV_UI_Height": 170,
 "ISV_UI_Weight": 60,
 "ISV_UI_Signature": "良辰必有重谢",
 "ISV_PT_TeachingProgram": "1,2",
 "ISV_PT_CourseCost": 1000
 },
 ...
 model	是	传入实体名称
 model.ISV_U_NickName	是	昵称
 model.ISV_UI_Age	是	年龄
 model.ISV_UI_Province	是	所在省份
 model.ISV_UI_City	是	所在城市
 model.ISV_UI_Job	否	职业
 model.ISV_UI_CaseHistory	否	过往病史
...
+ (void)userModifyWithAge:(NSUInteger )age
                     city:(NSString *)ISV_UI_City
                      job:(NSString *)ISV_UI_Job
              caseHistory:(NSString *)ISV_UI_CaseHistory
                   height:(NSUInteger )height
                   weight:(NSUInteger )weight
                signature:(NSString *)ISV_UI_Signature
                  ...
                  failure:(ErrorBlock)failure
{
    ...
    NSNumber *ISV_UI_Age = [NSNumber numberWithUnsignedInteger:age];
    NSNumber *ISV_UI_Height = [NSNumber numberWithUnsignedInteger:height];
    NSNumber *ISV_UI_Weight = [NSNumber numberWithUnsignedInteger:weight];
    if (IsStrEmpty(ISV_UI_Job)) {
        ISV_UI_Job = ISV_NULL;
    }
    if (IsStrEmpty(ISV_UI_CaseHistory)){
        ISV_UI_CaseHistory = ISV_NULL;
    }
    if (IsStrEmpty(ISV_UI_Signature)){
        ISV_UI_Signature = ISV_NULL;
    }
    // 2.拼接参数字典
    NSDictionary *model = NSDictionaryOfVariableBindings(ISV_UI_Age,
                                                         ISV_UI_City,
                                                         ISV_UI_Job,
                                                         ISV_UI_CaseHistory,
                                                         ISV_UI_Height,
...
#pragma - 修改昵称或头像
+ (void)userModifyWithNickname:(NSString *)ISV_U_NickName
                 headImageCode:(NSString *)ISV_u_headImage
                      success:(RespondBlock)success
                      failure:(ErrorBlock)failure
{
    NSString *key = [self token];
    NSString *whichFunc = @"UPDATEUSER";
    NSDictionary *model = NSDictionaryOfVariableBindings(ISV_U_NickName, ISV_u_headImage);
    NSDictionary *param = NSDictionaryOfVariableBindings(key,model,whichFunc);
    [self postWithURL:SETMYWALLETPASSWORD params:param success:^(id respondData) {
        success(respondData);
    } failure:^(ISVErrorModel *error) {
...
/*
 ...
 "ISV_UI_Sex": 0,   --性别
 "ISV_UI_Age": 19,   --年龄
 "ISV_UI_Province": "gd",   --所在省份
 "ISV_UI_City": "gz",   --所在城市
 "ISV_UI_Job": null,   --职业
 "ISV_UI_CaseHistory": null,   --过往病史
 "ISV_UI_Height": 0,   --身高
 "ISV_UI_Weight": 0,   --体重
 ...
 "ISV_PT_Qualification": 1,   --资质
 "ISV_PT_Team": "华山派",   --专业团队
 "ISV_PT_College": "笑傲江湖",   --毕业学校
 "ISV_PT_CourseCost": 60000,   --课时费
 "ISV_PT_ProfessionalProgram": 1,   --专业项目
 "ISV_PT_TeachingProgram": "独孤九剑",   --授课项目
 "ISV_PT_Province": "广东",   --所在省份
 "ISV_PT_City": "广州",   --所在城市
                    ...
                    failure:(ErrorBlock)failure
{
    ...
    // 1.获取自己的信息
    if ((userID==nil) || ([userID.description isEqualToString:ISV_NULL]))
    {
...
/*
 ...
 "ISV_UI_Sex": 1,   --性别
 "ISV_UI_Age": 18,   --年龄
 "ISV_UI_Province": "广东",   --所在省份
 "ISV_UI_City": "广州",   --所在城市
 "ISV_UI_Job": "IT程序猿",   --职业
 "ISV_UI_CaseHistory": "无",   --过往病史
...
+ (void)getListOfNearUserWithPage:(NSUInteger )page
                     ...
                     genderOption:(ISVSexType)gender
                       sortOption:(ISVSortOption)sortOption
                          success:(RespondBlock)success
                          failure:(ErrorBlock)failure
{
    // 根据key拼接url
...
/*
 ...
 key	是	用户登录后返回的token
 model	是	传入实体名称
 model.ISV_GNU_Longitude	是	用户经度
 model.ISV_GNU_Latitude	是	用户纬度
 whichFunc	是	决定调用哪个接口
 ...
 */
#pragma mark - 存储用户位置
+ (void)tellPositionWithLongitude:(NSString *)ISV_GNU_Longitude latitude:(NSString *)ISV_GNU_Latitude city:(NSString *)city success:(RespondBlock)success failure:(ErrorBlock)failure
{
    ...
    NSString *ISV_GNU_City = city ? city : kDefaultCityID;
    
    NSDictionary *model = NSDictionaryOfVariableBindings(ISV_GNU_Longitude, ISV_GNU_Latitude, ISV_GNU_City);
...
/*
 ...
 "key": "",
 "model": {
 "ISV_PTrainerId": "46646777",
 "ISV_ServerProgram": "八卦掌",
 "ISV_ServerTime": [
 "17",
 "18",
 "19"
 ],
 "ISV_ServerLocation": "天河",
 "ISV_UnitPrice": 100,
 "ISV_OrderFrom": "APP",
 "ISV_OrderType": 1,
 "ISV_Remark": "测试",
 "ISV_UserPhone": "13900000000",
 "ISV_serverdate": "2015-12-09"
 },
 ...
 model.ISV_UnitPrice	是	订单单价
 model.ISV_OrderFrom	是	订单来源
 model.ISV_OrderType	是	订单类型
 model.ISV_Remark	否	备注
 whichFunc	是	决定调用哪个接口
 ...
 */
#pragma mark - 用户下单
+ (void)userOrderWithCoachID:(NSString *)ISV_PTrainerId
                     serverProgram:(NSString *)ISV_ServerProgram
                     serverTime:(NSArray *)ISV_ServerTime
                     serverDate:(NSString *)ISV_serverdate
                     serverLocation:(NSString *)ISV_ServerLocation
                     price:(NSString *)OrderPrice
                     orderType:(NSString *)type
                     tel:(NSString *)ISV_UserPhone
                     remark:(NSString *)ISV_Remark
                     success:(RespondBlock)success failure:(ErrorBlock)failure
{
    ...
    NSString *ISV_OrderFrom = @"APP";
    NSNumber *ISV_OrderPrice = @(OrderPrice.integerValue);
    NSDictionary *model = NSDictionaryOfVariableBindings(ISV_PTrainerId,
                                                         ISV_ServerProgram,
                                                         ISV_ServerTime,
                                                         ISV_serverdate,
                                                         ISV_ServerLocation,
...
/*
 ...
 "ISV_ServerLocation": "天河",   --服务地址
 "ISV_UserPhone": "13900000001",   --用户手机
 "ISV_OrderPrice": 300,   --订单价格
 "ISV_OrderFrom": "APP",   --订单来源
 "ISV_PayStatus": "未付款",   --付款状态
 "ISV_OrderStatus": "未付款",   --订单状态
 "ISV_Remark": "测试",   --备注
 ...
 {
 "ISV_OrderId": "201510092232244834312",
 "User_Id": "25055247",
 "ISV_PTrainerId": "46646777",
 "ISV_ServerProgram": "八卦掌",
 "ISV_OrderDate": "2015-10-09T22:32:24.483595",
 "ISV_ServerTime": [
 "17",
 "18",
 "19"
...
/*
 ...
 MsgTime	操作完成后信息返回时间	2015-10-07T11:25:38.185579+08:00
 valuse	钱包信息	{
 "User_Id": "25055247",      "ISV_W_Amount": "0",
 "ISV_W_Point": "0",
 "ISV_W_PayPwd": "123"
 }
 */
...
/*
 ...
 "model": {
 "ISV_WA_Amount": 100,
 "ISV_WA_Flag": "65559998941235656488",
 "ISV_wa_rateamount": 2.00
 },
 ...
 key	是	用户登录后返回的token
 model	是	传入实体名称
 model.ISV_WA_Amount	是	申请提现金额
 model.ISV_WA_Flag	是	银行卡号
 ISV_wa_rateamount	是	手续费
 whichFunc	是	决定调用哪个接口
...
+ (void)userWithdrawBankCardFlag:(NSString *)flag amount:(NSString *)amount rateamount:(NSString *)FEE success:(RespondBlock)success failure:(ErrorBlock)failure{
    ...
    NSMutableDictionary* model = [[NSMutableDictionary alloc]init];
    [model setValue:flag forKey:@"ISV_WA_Flag"];
    [model setValue:amount forKey:@"ISV_WA_Amount"];
    
    [model setValue:FEE forKey:@"ISV_wa_rateamount"];
...
/*
 ...
 "key": "",
 "model": {
 "ISV_WR_OrderId": "wa201510151955333588350",
 "ISV_WR_WithdrawDate": "2015-10-15 10:15:36",
 "ISV_WR_IsPass": true,
 "ISV_WR_Auditor": "haha",
 "ISV_WR_IsReceive": true,
 "ISV_WR_SerialNumber": "liushuizhanghao123"
 },
 ...
 key	是	用户登录后返回的token
 model	是	传入实体名称
 model.ISV_WR_OrderId	是	订单号
 model.ISV_WR_WithdrawDate	是	提现时间
 model.ISV_WR_IsPass	是	是否通过
 model.ISV_WR_Auditor	是	审核人
 model.ISV_WR_IsReceive	是	是否到账
 model.ISV_WR_SerialNumber	是	流水账号
 whichFunc	是	决定调用哪个接口
...
/*
 ...
 "ISV_orderid": "wa201601241356581897777", --订单id或提现id
 "ISV_inr_amountbefore": 196, --订单前金额
 "ISV_inr_amount": 102, --订单金额
 "ISV_inr_amountafter": 298, --订单后金额
 "ISV_inr_date": "2016-01-24T16:09:07.412305", --成功付款日期
 "ISV_inr_serveenddate": "0001-01-01T00:00:00", --已服务日期
 "ISV_inr_receivedate": "0001-01-01T00:00:00", --已到账日期
 "ISV_inr_refunddate": "0001-01-01T00:00:00", --退款成功日期
 "ISV_inr_status": 0, --收入状态，0：暂无，1：成功付款，2：已服务，3：成功到账，4：已退款；支出状态，1：提交申请，2：审核中，3：提现成功，4：提现不成功
 "ISV_wa_bankcard": "012345678912345678901", --银行卡
 "ISV_wa_status": "-2", --提现金额
 "ISV_wa_date": "2016/1/24 13:56:58", --提现状态
 "ISV_wa_rateamount": "2", --提现费率
 "inorout":1--1：收入，2：支出
 }
 ]
 }
...
/*
 ...
 key	是	用户登录后返回的token
 model	是	传入实体名称
 model.ISV_orderid	是	订单号
 model.ISV_ptc_score	是	评分
 model.ISV_ptc_content	是	评论内容
 whichFunc	是	决定调用哪个接口
...
+ (void)userEvaluateCoachScore:(NSString *)ISV_ptc_score content:(NSString *)ISV_ptc_content orderId:(NSString *)ISV_orderid type:(NSString *)type success:(RespondBlock)success failure:(ErrorBlock)failure{
    NSString *key = [self token];
    NSString *whichFunc = @"IN_PTCOMMENT";
    NSDictionary *model = NSDictionaryOfVariableBindings(ISV_orderid, ISV_ptc_score, ISV_ptc_content);
...
/*
 ...
 tbid	是	活动id
 user_id	是	参与活动用户id
 ISV_ucou_coupon	是	优惠券信息
 ISV_ujr_award	否	奖品说明
...
/*
 ...
 "tbid": 0,
 "user_id": "123456",
 "ISV_ucou_coupon": "{\"ISV_ucou_coupon\": [{\"IsUsed\": true, \"EndDate\": \"2015-12-11T00:20:00\", \"CouponId\": \"0af528674b324b248824f39aee19d550\", \"Discount\": 0.9, \"MaxMoney\": 100.0, \"MinMoney\": 10.0, \"StartDate\": \"2015-12-11T00:20:00\", \"CouponName\": \"活动有礼\"}]}",
 "ISV_ujr_award": null
 }
 */
...
/*
 ...
 {
 "ISV_bankl_id": 1,--递增id
 "ISV_bankl_name": "中国银行",--银行名称
 "ISV_bankl_image": null,--图片
 "ISV_bankl_sort": 1--排序ID
 },
 {
 "ISV_bankl_id": 2,
 "ISV_bankl_name": "民生银行",
 "ISV_bankl_image": null,
 "ISV_bankl_sort": 2
 },
 {
 "ISV_bankl_id": 3,
 ...
 {
 "ISV_bankl_id": 4,
 "ISV_bankl_name": "中国农业银行",
 "ISV_bankl_image": null,
 "ISV_bankl_sort": 4
 },
 {
 "ISV_bankl_id": 5,
 "ISV_bankl_name": "中国建设银行",
 "ISV_bankl_image": null,
 "ISV_bankl_sort": 5
 },
 {
 "ISV_bankl_id": 6,
 "ISV_bankl_name": "中国工商银行",
 "ISV_bankl_image": null,
 "ISV_bankl_sort": 6
 },
 {
