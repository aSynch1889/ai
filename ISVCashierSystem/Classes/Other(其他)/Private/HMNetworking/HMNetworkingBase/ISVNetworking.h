...
FOUNDATION_EXPORT NSString *const ISV_NULL;
 ...
 *
 *  注意:
 *        1.ISVNetworking要求所有参数必须是`对象`类型 (回调的block除外)
 *        2.如果参数为空, 使用`ISV_NULL` （参照ISVHomeViewController.m中的示例）
 *
 *********************************************************************************/


typedef void (^RespondBlock)(id respondData);
typedef void (^ErrorBlock)(ISVErrorModel *error);


@interface ISVNetworking : NSObject
/// 登录成功后的用户令牌
...
/**
 * `返回令牌` 注意:token可能为nil, 
 * 使用NSDictionaryOfVariableBindings(以下简称DOB)拼接参数时,需要手动转为ISV_NULL
 * 建议使用DOB时, token单独拼接
 */
+ (NSString *)token;
