...
/*
 ...
 signature的加密：
 C# 下操作演示
 string[] ArrTmp = { model.HM_U_Pwd, models.timestamp, models.nonce };
 Array.Sort(ArrTmp);
...
+ (void)uploadImageWithPaths:(NSArray *)imagePaths userID:(NSString *)User_Id success:(RespondBlock)success failure:(ErrorBlock)failure
{
    ...
    // 5.判断是否头像
    NSString *type = [User_Id isEqualToString:HM_NULL] ? HM_NULL : @"UpPhoto";
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        for (NSString *path in imagePaths)
        {
            ...
            if (err)
            {
                HMErrorModel *errModel = [[HMErrorModel alloc] init];
                errModel.error = err;
...
{
    NSString *orderTypeStr = HMOrderTypeDesc[@(OrderType)];
...
/*
 ...
 {
 "hm_ad_id": 2, --广告id
 "hm_ad_title": "ad2", --广告标题，供后台区分，app不需要
 "hm_ad_picurl": "picurl2", --广告图片地址
 "hm_ad_interval": 10, --显示时间
 "hm_ad_canclick": true, --是否可点击
 "hm_ad_clickurl": "clickurl2", --点击后链接的url
 "hm_ad_status": 1, --状态，0-->不显示，1-->显示
 "hm_ad_createtime": "2016-01-25T16:51:03.338264"--创建时间
 }
 ]}
 参数说明：
 参数	说明	值
...
+ (void)commonKeyWithSuccess:(RespondBlock)success failure:(ErrorBlock)failure
{
    [self getWithURL:CommonKey success:^(id respondData) {
        ...
        [[[HMNetworking alloc] init] performSelectorInBackground:@selector(setToken:) withObject:token];
        
        success(respondData);
        
    } failure:^(HMErrorModel *error) {
        
        [[[HMNetworking alloc] init] performSelectorInBackground:@selector(setToken:) withObject:kDefaultKey];
